...
public interface travelsrepo extends JpaRepository<travelsmodel, Integer>{
	//native Query
		...
		@Query(value="select * from garuda", nativeQuery = true)
		public List<travelsmodel> getAllRows();
		
		@Query(value="select * from garuda where travelsname like %?1%",nativeQuery = true)
		public List<travelsmodel> getByname(@Param("name") String name);
			
		@Query(value="select * from garuda where Destination=:dest and travelsname=:name",nativeQuery = true )
		...
		@Modifying
		@Transactional
		@Query(value="delete from garuda where travels_id=:id",nativeQuery=true)
		...
		@Modifying
		@Transactional
		@Query(value="update garuda set destination=:dest where travels_id=:id",nativeQuery = true)
